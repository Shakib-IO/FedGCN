...
def test(model, features, adj, labels, idx_test):
    ...
    return loss_test.item(), acc_test.item()#auc_test.item()

def eval_metrics(model, features, adj, labels, idx_test):
    model.eval()
    output = model(features, adj)
    pred_labels=torch.argmax(output,axis=1)
    
    #evaluation metrics -> Precision, Recall, Balanced Accuracy, and F1-score. 
    precision_score = metrics.precision_score(labels[idx_test].cpu().detach().numpy(), pred_labels[idx_test].cpu().detach().numpy(), average='weighted')
    recall_score = metrics.recall_score(labels[idx_test].cpu().detach().numpy(), pred_labels[idx_test].cpu().detach().numpy(), average='weighted')
    balanced_acc = metrics.balanced_accuracy_score(labels[idx_test].cpu().detach().numpy(), pred_labels[idx_test].cpu().detach().numpy())
    ...
    return precision_score.item(), recall_score.item(), balanced_acc.item(), f1_test.item(), #, auc_test.item()
...
def train(epoch, model, optimizer, features, adj, labels, idx_train):  #Centralized
    ...
    output = model(features, adj)
    #Modify
    # output[idx_train] = torch.LongTensor(output[idx_train])
    # labels[idx_train] = torch.LongTensor(labels[idx_train])

    # output[idx_train] = output[idx_train].type(torch.float64)
    # labels[idx_train] = labels[idx_train].type(torch.float64)

    loss_train = F.nll_loss(output[idx_train], labels[idx_train].long())#.cpu().detach()
    acc_train = accuracy(output[idx_train], labels[idx_train])#.cpu().detach()
...
def Lhop_Block_matrix_train(epoch, model, optimizer, features, adj, labels, communicate_index, in_com_train_data_index):
    ...
    loss_train = F.nll_loss(output[in_com_train_data_index], labels[communicate_index][in_com_train_data_index].long())
